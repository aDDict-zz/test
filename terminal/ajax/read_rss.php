<?php
	session_start();
	include_once('../../www/libs/Smarty.class.php');
	include_once('../../www/inc/lastRSS.php');
	include_once('../../www/inc/globals.php');
	include_once('../../www/lang/'.$__lang.'/error.php');
	include_once('../../www/inc/feed_functions.php');
	...
	#$__user_id = 1;
if (isset($_REQUEST["debug"]) && $_REQUEST["debug"] == "apro") {
	...
	switch($_REQUEST['action']){		
		case "read_rss":
			...
			if($feed != ""){													
                ...
                $afeed = explode(":|:", $feed);
                if ($type == 1 && count($afeed) > 1) $type = 3;
                if ($type == 3) {
                    $sql = "select * from fixed_categories where cat_rss = '$feed'";
                    $r = mysql_query($sql);
                    if ($row = mysql_fetch_assoc($r)) {
                        $titles = explode(":|:", $row["cat_rss_titles"]);
                        $newsperfeed = $row["cat_newsperfeed"];
                    } else {
                        $newsperfeed = 3;
                    }
                    $multi = 1;
                    $tpl = "rss_list_multi.html";
                    $aresult = array();
                    for ($i = 0; $i < count($afeed); $i++) {
                        $idx = $i;
                        $fee = $afeed[$i];
                        if (!$error && !empty($fee)) {
                            $url = 'http://'.parseURL($fee);
                            ...
                            if($news_id || !$smarty->is_cached($tpl, md5($url))) { 
                                if(parseFeed($url, $aresult[$idx], $news_order)) {
                                    $aresult[$idx]["entries"] = array_slice($aresult[$idx]["entries"], 0, $newsperfeed);
                                    $assign = 1;
                                } else {
                                    $error = 1; 
                                }
                            }
    #print "<pre>";print_r($aresult[$idx]); print "</pre>";
                        }
                    }
                } elseif ($type == 1) {
                ...
                if ($error) {
                    ...
                    if ($assign) {
                        if ($multi) {
                            ...
                            $smarty->assign('results', $aresult);
                            $smarty->assign('items_nr', $newsperfeed);	
                            if ($marked_news) {
                                $smarty->caching = 0;
