...
find "$TR_DOWNLOADED_PATH" -name \*.avi -exec mv {} "$target.avi" \;
find "$TR_DOWNLOADED_PATH" -name \*.iso -exec mv {} "$targetdir.iso" \;
find "$TR_DOWNLOADED_PATH" -name \*.mkv -exec mv {} "$targetdir.mkv" \;
