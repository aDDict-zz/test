...
function showNewFeedBox(r, data){	
	...
	if(activeDefaultTab!=null)	{
        mainContent = document.getElementById('dfp_' + activeDefaultTab.id.substring(3, activeDefaultTab.id.length));		
    } else {
        mainContent = document.getElementById('usp_s' + activeTab.id.substring(3, activeTab.id.length));
    }
    ...
    var bid = data['maxBoxId'];
    var cell_idx = 0;
    if (r.responseText.match(/alt="bid_(\d+)_(\d+)"/m)) {
        bid = parseInt(RegExp.$1);
        cell_idx = parseInt(RegExp.$2);
//        alert('bid: ' + bid+ '\ncl:'+cell_idx);
    }
    var firstCol;
    var before;
	...
	if(IE){
        ...
        firstCol = findColDiv(0, cell_idx);
		before = firstCol.childNodes[0];
	} else {	
        ...
        if (firstCol) {
            ...
            if(mainContent.rows[0].cells[cell_idx].childNodes.length==1){
                firstCol = mainContent.rows[0].cells[cell_idx].childNodes[0];						
                before = null;
            }else{
                firstCol = mainContent.rows[0].cells[cell_idx].childNodes[1];
                before = firstCol.childNodes[1];
            }
        }
	}
