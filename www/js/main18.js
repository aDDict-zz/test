...
//        alert('Page id: '+pageId+'\nactivetab.id:'+activeTab.id.substring(3, activeTab.id.length));
	...
	this.build = function(){				
		...
		if(IE){
            ...
            var contentCloseLink = editDiv.childNodes[3];
            var contentCloseIcon = contentCloseLink.childNodes[0];
			if(closeable==1) {
                var closeLink;
                if (!(closeLink = editDiv.childNodes[4])) closeable = 0;
            }
...
function createPage(r, div){			
   ...
   evalScriptFromHtml(r.responseText, div);
    ...
    var o;
    for (var ij=0;ij<fileContents.length;ij++) {
        var key = fileContents[ij][0];
        var file = fileContents[ij][1];
