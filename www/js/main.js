...
var Box = function(obj, title, feed, contentClosed, itemsNr, type, color, editable, moveable, closeable, pageId){				
	...
	this.dragEnd = function(pageId){
		...
		if(activeDefaultTab==null)	var pars = "action=update_strucure&page_id=" + activeTab.id.substring(3, activeTab.id.length) + '&firstCol=' + encodeURIComponent(columns[0])  + '&secondCol=' + encodeURIComponent(columns[1]) + '&thirdCol=' + encodeURIComponent(columns[2]);
			else var pars = "action=update_strucure&default=1&page_id=" + activeDefaultTab.id.substring(3, activeDefaultTab.id.length) + '&firstCol=' + encodeURIComponent(columns[0])  + '&secondCol=' + encodeURIComponent(columns[1]) + '&thirdCol=' + encodeURIComponent(columns[2]);										
	...
	this.build = function(){				
		...
		if(IE){
			...
			if(editable==1)	var editLink = editDiv.childNodes[2];
			if(closeable==1){				
				switch(parseInt(type)){
					case 1:var closeLink = editDiv.childNodes[3];break;
					case 2:var closeLink = editDiv.childNodes[2];break;
					case 3:var closeLink = editDiv.childNodes[2];break;
				}	
			}
			...
			if(type==1){				
				...
				var cWhite = editContent.childNodes[0].childNodes[0].childNodes[1].childNodes[1].childNodes[0];			
				var cYellow = editContent.childNodes[0].childNodes[0].childNodes[1].childNodes[1].childNodes[1];
				var cGreen = editContent.childNodes[0].childNodes[0].childNodes[1].childNodes[1].childNodes[2];
				var cPink = editContent.childNodes[0].childNodes[0].childNodes[1].childNodes[1].childNodes[3];
				var cBlue = editContent.childNodes[0].childNodes[0].childNodes[1].childNodes[1].childNodes[4];
				var cOrange = editContent.childNodes[0].childNodes[0].childNodes[1].childNodes[1].childNodes[5];
			...
			if(editable==1)	var editLink = editDiv.childNodes[2];
			if(closeable==1) var closeLink = editDiv.childNodes[3];
			...
			if(type==1){							
				...
				var cWhite = editContent.childNodes[1].childNodes[1].childNodes[2].childNodes[3].childNodes[1];
				var cYellow = editContent.childNodes[1].childNodes[1].childNodes[2].childNodes[3].childNodes[3];
				var cGreen = editContent.childNodes[1].childNodes[1].childNodes[2].childNodes[3].childNodes[5];
				var cPink = editContent.childNodes[1].childNodes[1].childNodes[2].childNodes[3].childNodes[7];
				var cBlue = editContent.childNodes[1].childNodes[1].childNodes[2].childNodes[3].childNodes[9];
				var cOrange = editContent.childNodes[1].childNodes[1].childNodes[2].childNodes[3].childNodes[11];			
		...
		if(type==1){
			...
			cYellow.onclick = function(){
				color = 1;
				h1.style.background = "#FFFFE0";			
			}
			cGreen.onclick = function(){
				color = 2;
				h1.style.background = "#EEFFE0";			
			}
			cPink.onclick = function(){
				color = 3;
				h1.style.background = "#FFE0E1";			
			}
			cBlue.onclick = function(){
				color = 4;
				h1.style.background = "#EFF5FF";			
			}
			cOrange.onclick = function(){
				color = 5;
				h1.style.background = "#FEE8BD";			
			}
			...
			updateButton.onclick = function(){			
				...
				if(activeDefaultTab==null)	var pars = "action=update_box&page_id=" + activeTab.id.substring(3, activeTab.id.length) + "&box_id=" + obj.getAttribute('alt') + "&box_title=" + encodeURIComponent(inputTitle.value) + '&box_feed=' + encodeURIComponent(inputFeed.value) + "&box_color=" + color + "&box_items_nr=" + itemsNr;
					else var pars = "action=update_box&default=1&page_id=" + activeDefaultTab.id.substring(3, activeDefaultTab.id.length) + "&box_id=" + obj.getAttribute('alt') + "&box_title=" + encodeURIComponent(inputTitle.value) + '&box_feed=' + encodeURIComponent(inputFeed.value) + "&box_color=" + color + "&box_items_nr=" + itemsNr;
				var url = "ajax/page.php";
				Ajax.send(url, "POST", pars, null, null);
		...
		if(contentClosed=="0")	favIcon.onclick = hideContent;
			else favIcon.onclick = showContent;
		
		var favIconSrc = favIcon.src;
		
		var boxHeadOver = function(){
			if(contentClosed=="0")	favIcon.src = 'i/va.gif';
				else favIcon.src = 'i/ha.gif';
		...
		function getRSS(onRefresh, force){
			var pars = "action=read_rss&feed=" + feed + "&items_nr=" + itemsNr + '&type=' + type + '&force=' + force;			
...
function createPage(r, div){			
	...
	var mainContent = div;	
	var pageId = div.id.substring(4, div.id.length);
	...
	if(fixedCategories!=null){
		for (var catID in fixedCategories){		
			...
			var pars = "action=read_rss&feed=" + encodeURIComponent(fixedCategories[catID]);			
			var url = "ajax/read_rss.php";
	...
	for(var i=0;i<components.length;i++){		
		for(var j=0;j<components[i].length;j++){			
			boxes[components[i][j]['title']] = new Box(document.getElementById('comp_' + pageId + '_' + i + '_' + j), components[i][j]['title'], components[i][j]['feed'], components[i][j]['closed'], components[i][j]['items_nr'], components[i][j]['type'], components[i][j]['color'], components[i][j]['editable'], components[i][j]['moveable'], components[i][j]['closeable'], pageId);
		}
	}		
}
...
function showDefaultPage(pageID, tab){
	...
	if(activeTab){
		...
		if(document.getElementById('usp_' + oldPageId)) document.getElementById('usp_' + oldPageId).style.display = 'none';
		if(IE){
	...
	if(activeDefaultTab){
		activeDefaultTab.style.color = "#00368F";
...
var createTabs = function(r){		
	...
	showDefaultPage(1, document.getElementById('dp_1'));
}
...
var closeTab = function(tab){
	if(confirm(lang['onTabDelete'])){
		...
		tabItems.length--;
		var mainContent = document.getElementById('mainContent');		
		mainContent.removeChild(document.getElementById('usp_' + tab.id.substring(3, tab.id.length)));
		...
		if(!document.getElementById('usp_' + activeTab.id.substring(3, activeTab.id.length))){
			var div = document.createElement('div');
			div.id = 'usp_' + activeTab.id.substring(3, activeTab.id.length);
			mainContent.appendChild(div);
			
			var pars = "action=get_page_structure&page_id=" + activeTab.id.substring(3, activeTab.id.length);
			var url = "ajax/page.php";
			Ajax.send(url, "POST", pars, createPage, div);	
		}else document.getElementById('usp_' + activeTab.id.substring(3, activeTab.id.length)).style.display = 'block';
	}
}
...
var Tabs = function(tab, title){
	...
	tab.onclick = function(){				
		if(tab.className=='tabinactive'){										
			...
			tab.appendChild(img);
			if(!document.getElementById('usp_' + tab.id.substring(3, tab.id.length))){
				var div = document.createElement('div');
				div.id = 'usp_' + tab.id.substring(3, tab.id.length);
				var mainContent = document.getElementById('mainContent');
				mainContent.appendChild(div);
				...
				var url = "ajax/page.php";
				Ajax.send(url, "POST", pars, createPage, div);	
			}else	document.getElementById('usp_' + tab.id.substring(3, tab.id.length)).style.display = 'block';
