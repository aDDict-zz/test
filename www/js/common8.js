...
function insertMonddMeg() {
	...
	if (div) {
		var category = 0;
		if (this.activeDefaultTab) {
			pageId = parseInt(this.activeDefaultTab.id.replace('dp_', ''));
		}
		...
		if(categories[pageId]) {
			category = categories[pageId];
		} else {
			category = categories[1];
		}
		var e = document.createElement('script');
		var src = 'http://www.monddmeg.hu/api/mm.js?site_id=5&category=%category&w=300&id=monddmeg_doboz';
