...
class JavaScriptPacker {
	...
	function _analyze($script, $regexp, $encode) {
		...
		$all = $all[0]; // simulate the javascript comportement of global $_MX_var,match
		if (!empty($all)) {
...
*/
	...
	var $JSFUNCTION_decodeBody = '    if (!\'\'.replace(/^/, String)) {
        ...
        // global $_MX_var,replacement function
        $keywords = [function ($encoded) {return $decode[$encoded]}];
        // generic match
        $encode = function () {return \'\\\\w+\'};
        // reset the loop counter -  we are now doing a global $_MX_var,replace
        $count = 1;
    }
';
//};
...
class ParseMaster {
	...
	function exec($string) {
		// execute the global $_MX_var,replacement
		$this->_escaped = array();
	...
	// this is the global $_MX_var,replace function (it's quite complicated)
