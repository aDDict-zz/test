...
;(function ($) {
	...
	$.modal.impl = {
		...
		init: function (data, options) {
            ...
            if ($("img#simplemodal-data").length == 0) {
                $("#get_help").trigger("click");
            }
		...
		setContainerDimensions: function () {
			...
			if (dh > ch || dw > cw) {
				this.dialog.wrap.css({overflow:'hidden'});
			}
			this.setPosition();
		},
		...
		close: function () {
			...
			if ($.isFunction(this.opts.onClose) && !this.occb) {
				...
				$("#basic-modal-content").addClass('vishid');		
...
function resize_modal() {
    ...
    var img = 0;
    if ($("img#simplemodal-data").length > 0) {
        var content = $("#simplemodal-data");
        $("#simplemodal-data").removeClass();
        var plus = 0;
        var defwidth = 0;
        var defheight = 0;
	img = 1;
        ...
        var content = $("#basic-modal-content");
        var plus = 30;
        var defwidth = 600;
        var lngcount = $("input#lng_count").val();
        var defheight = lngcount * 210;
    }
    var w = content.width()+plus;
    if (w < minwidth) w = minwidth;
    if (defwidth > 0) w = defwidth;
    if (w > maxwidth) {
        w = maxwidth;
    }

    var h = content.height()+plus;
    if (h < minheight) h = minheight;
    if (defheight > 0) h = defheight;
    if (h > maxheight) {
        h = maxheight;
    }    

    var t = (($(window).height() - h ) / 2)-15;
    var l = (($(window).width() - w ) / 2)-15;

    $("#simplemodal-container").animate({width : w+'px',height : h+'px',top : t+'px',left : l+'px'},500,function(){
        if($("#basic-modal-content").attr('class') == 'vishid') {
            $("#basic-modal-content").removeClass();
        }
	if (img == 0) $("#basic-modal-content").attr('style','display:block');
	else $("#basic-modal-content").removeAttr('style');
        $(".wysiwyg").show();
        $("#basic-modal-content").removeClass();
        $("div.simplemodal-wrap").css({height: '100%', outline: 0, width: '100%', overflow: 'auto'}); });
