...
function jscolor_init() {
	...
	function createDialog() {
		...
		setStyle(elements.dialog, {
			...
			'height' : HVSize[1]+2*padding+20+'px',
		...
		elements.trans = document.createElement('div')
		setStyle(elements.trans, {
			'position' : 'absolute',
			'left' : '0',
			'top' : HVSize[1]+padding+5+'px',
			'width' : SSize+padding+'px',
			'height' : '20px',
		})            
		var set_trans = function(e) {
			updateInput(instance.input, '', null)
		}        
		elements.trans.onmousedown = function(e) { instance.holdS = true; set_trans(e) }        
        elements.trans.innerHTML="transzparens";
        elements.trans.id="jscolor_trans_link";
		elements.dialog.appendChild(elements.trans);                   
	}
	...
	function showDialog(input) {
        ...
        var trans_vis='visible';
        if (tmp=="border") trans_vis='hidden';
        elements.dialog.childNodes[3].style.visibility=trans_vis;
