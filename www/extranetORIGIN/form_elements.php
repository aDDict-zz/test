...
function toggle_options(on) {
    location = 'form_in.php?group_id=$group_id&form_id=$form_id&toggle_options=' + on;
}
...
if (isset($_GET["toggle_options"])) {
    if ($_GET["toggle_options"]==1) {
        $errorlist=$word["fe_show_all_options_set"];
    }
    else {
        $errorlist=$word["fe_show_no_options_set"];
    }    
}

if (!empty($errorlist)) {
    print "<tr><td colspan='6' bgcolor='white' style='padding:24px;'><span class='szovegvastag'>$errorlist</span></td></tr>\n";
}
...
echo "<tr><td class='bgkiemelt2' align=left width=13%><span class=szovegvastag>$word[t_varname]</span></td>
      ...
      <a href='#' onClick='toggle_mandatory(0); return false;'>$word[fe_mandatory_none]</a>
        <br>
      <a href='#' onClick='toggle_options(1); return false;'>$word[fe_show_all_options]</a>
        &nbsp;
      <a href='#' onClick='toggle_options(0); return false;'>$word[fe_show_no_options]</a>
      </span></td>
      </tr>\n";
