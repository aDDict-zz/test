...
$res=mysql_query("select count(*) from sms_send where group_id='$group_id' and test='no'");
...
switch($sortt) {
    case 1: $order = "order by message asc"; break;
    case 2: $order = "order by message desc"; break;
    case 3: $order = "order by create_time asc"; break;
    case 4: $order = "order by create_time desc"; break;
    case 5: $order = "order by name asc"; break;
    case 6: $order = "order by name desc"; break;
}
  
if ($spec==2) 
    $spectext="spec_received_sms_";
    //$spectext="spec_ct_";
else
    $spectext="spec_sms_message_";
          
$res=mysql_query("select sms_send.*, user.name from sms_send,user where sms_send.sender_id=user.id 
                  and group_id='$group_id' and test='no' $order limit $first,$maxPerPage");
...
if ($res && mysql_num_rows($res)) { 
    ...
    print "
        <TR align=middle>
        <TD class=bgkiemelt2 width='20%'><span class='szovegvastag'>$word[t_id]</span></TD>
        <TD class=bgkiemelt2 width='20%'><span class='szovegvastag'>$word[t_date]</span></TD>
        <TD class=bgkiemelt2 width='20%'><span class='szovegvastag'>$word[t_sender]</span></TD>
        <TD class=bgkiemelt2 width='40%'><span class='szovegvastag'>$word[t_message]</span></TD>
        </TR>\n";
    while($row=mysql_fetch_array($res)) {
        if (empty($row['message']))
            $message="[ $word[no_message] ]"; 
        else  
            $message=$row['message'];
        if ($row["test"]=="yes") {
        ...
        print "<TR>
               <TD align=middle $tdclass vAlign=top width='20%'>
               <SPAN class=szoveg>$row[id]&nbsp;<a href=\"javascript:$jsfunc\">$word[select_mess]</a>&nbsp;</span></TD>
               <TD align=middle $tdclass vAlign=top width='20%'>
               <SPAN class=szoveg>$row[create_time]&nbsp;$testaddon</span></TD>
               <TD align=middle $tdclass vAlign=top width='20%'>
               <SPAN class=szoveg>"
               ...
               .nl2br(htmlspecialchars(quoted_printable_decode(decode_mime_string($message))))."\n";
        if ($ismime) {
  ...
  $params="group_id=$group_id&spec=$spec&wizard=$wizard";  
    ...
    if ($first>0)
        echo "<td nowrap align='right'>
           <a href='$_MX_var->baseUrl/sms_message_select.php?$params&first=0'><img src='$_MX_var->baseUrl/$_MX_var->application_instance/gfx/down2.gif' width='20' height='14' border='0'></a>
           <a href='$_MX_var->baseUrl/sms_message_select.php?$params&first=$OnePageLeft'><img src='$_MX_var->baseUrl/$_MX_var->application_instance/gfx/down1.gif' width='20' height='14' border='0'></a>
           </td>\n";
    else
        echo "<td nowrap align='right'>&nbsp;&nbsp;
    ...
    if ($first<$LastPage)
        echo "<td nowrap align='right'> &nbsp;
            <a href='$_MX_var->baseUrl/sms_message_select.php?$params&first=$OnePageRight'><img src='$_MX_var->baseUrl/$_MX_var->application_instance/gfx/up1.gif' width='20' height='14' border='0'></a>
            <a href='$_MX_var->baseUrl/sms_message_select.php?$params&first=$LastPage'><img src='$_MX_var->baseUrl/$_MX_var->application_instance/gfx/up2.gif' width='20' height='14' border='0'></a>
            &nbsp; </td>\n";
    else
        echo "<td nowrap align='right'>&nbsp;&nbsp;
