<?php
    ...
    $newsc = array(2, 3, 4, 5, 6, 7, 8, 9,10);
	...
	switch($_POST["action"]){
		...
		case "update_cat_html":
			$html = mysql_real_escape_string(trim($_POST['html']));
			...
			break;
		case "update_cat_keyword":
			$cat_id = $_POST['cat_id'];
            $keyword = $_POST["keyword"];
            $keyword = mysql_real_escape_string($keyword);
			if(!empty($keyword) && !empty($cat_id)) {
				$query = "Update fixed_categories Set cat_keyword='".$keyword."' Where cat_id=".$cat_id." Limit 1";
				mysql_query($query) or die(mysql_error());
            }
		...
		case "update_cat_rss":
			...
			break;
		case "update_cat_multirss":
			$rss_url = $_POST['rss_url'];
			$rss_titles= $_POST['rss_titles'];
            $newsperfeed = $_POST["newsperfeed"];
			$cat_id = $_POST['cat_id'];
			if($rss_url!='' && $cat_id!=''){
				$query = "Update fixed_categories Set cat_rss='$rss_url', cat_rss_titles = '$rss_titles', cat_newsperfeed= '$newsperfeed' Where cat_id=".$cat_id." Limit 1";
				mysql_query($query) or die(mysql_error());
			}
			break;
		case "remove_cat":
		...
		case "get_category":
			...
			if($cat_id!=''){
				...
				$smarty->assign('cat_newsperfeed', $row['cat_newsperfeed']);
                $multi_rss = explode(":|:", $row["cat_rss"]);
                if (count($multi_rss) < 5) {
                    $multi_rss = array("","","","","");
                }
                $multi_titles = explode(":|:", $row["cat_rss_titles"]);
                if (count($multi_titles) < 5) {
                    $multi_titles = array("","","","","");
                }
                $smarty->assign("multi_titles", $multi_titles);
                $smarty->assign("multi_rss", $multi_rss);
                $smarty->assign('cat_rss', $row['cat_rss']);
				$smarty->assign('cat_html', stripslashes($row['cat_html']));
				$smarty->assign('cat_favicon', $row['cat_favicon']);
				$smarty->assign('cat_keyword', $row["cat_keyword"]);
		...
		case "resort_page":
			$page_id  = $_POST['page_id'];
			if(1 || $page_id!=-1){
				$col_one_cat = explode(',', $_POST['column_one'][0]);			
				$col_two_cat = explode(',', $_POST['column_two'][0]);			
				$categories = array(
					'0'=>$col_one_cat,
					'1'=>$col_two_cat
				);
				if($_POST['column_one'][0]=='') unset($categories[0]);
		...
		case "update_cat":
			...
			$type = $_POST['type'];
            $cat_favicon = trim($_POST['cat_favicon']);
			if($cat_id!='' && $cat_name!=''){
				$query = "Update fixed_categories Set cat_name='".$cat_name."', cat_type=".$type.", cat_favicon='".$cat_favicon."' Where cat_id=".$cat_id." Limit 1";
				mysql_query($query) or die(mysql_error());
			}
			break;
		case "add_new_cat":
