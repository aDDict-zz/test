...
class MxVars {
    ...
    private $versions = array(
        ...
        "kc"=>array("live"=>array(
                            ...
                            "altDomains"=>array("beta.kutatocentrum.hu","alfa.kutatocentrum.hu","www.researchcenter.hu","beta.researchcenter.hu","alfa.researchcenter.hu"),
    ...
    public $db = array("main"=>array("host"=>"localhost","user"=>"root","password"=>"v","name"=>"maxima","name_public"=>"maxima"),
                       "bounce"=>array("host"=>"localhost","user"=>"root","password"=>"v","name"=>"maxima","name_public"=>"maxima"));
        ...
        foreach ($this->versions as $instance=>$data) {
            ...
            if ($index=="live" && isset($data["live"]["altDomains"])) {
                foreach ($data["live"]["altDomains"] as $alt) {
                    if ($this->getRootDomain($_SERVER["HTTP_HOST"])==$alt || $_SERVER["HTTP_HOST"]==$alt) {
                        $this->setVersionData($instance,$data,$index);
                    }
                }
            }
