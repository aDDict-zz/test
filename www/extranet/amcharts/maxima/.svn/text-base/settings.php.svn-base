...
<settings> 
  ...
  <values>                                                    <!-- VALUES -->
    <category>                                                <!-- category axis -->
      ...
      <frequency><?=$labelFreq;?></frequency>                                <!-- [1] (Number) how often values should be placed -->
  ...
  <labels>                                                    <!-- LABELS -->
    ...
    <label lid="0">
      ...
      <text>                                                  <!-- [] (text) html tags may be used (supports <b>, <i>, <u>, <font>, <a href="">, <br/>. Enter text between []: <![CDATA[your <b>bold</b> and <i>italic</i> text]]>-->
        <![CDATA[<b><?=$graphTitle;?><?php if ( $type_id != "demogdate" ) { ?> (<?=$startdate;?> - <?=$enddate;?>)<?php } ?></b>]]>
      </text>        
    </label>
  </labels>
  ...
  <graphs>                                                     <!-- GRAPHS SETTINGS. These settings can also be specified in data file, as attributes of <graph>, in this case you can delete everything from <graphs> to </graphs> (including) -->
    ...
    <graph gid="1">                                            <!-- if you are using XML data file, graph "gid" must match graph "gid" in data file -->
      ...
      <title><?=$graphMeasure;?></title>                                   <!-- [] (graph title) -->
      ...
      <balloon_text>
        <![CDATA[<?=$graphUnit;?> <?php if ( $type_id != "demogdate" ) { ?><?=$word['datedef'];?> <?php } ?>{series}: {value} <?=$graphMeasure;?>]]>              <!-- [column(line).balloon.text] ({title} {value} {series} {description} {percents}) You can format any balloon text: {title} will be replaced with real title, {value} - with value and so on. You can add your own text or html code too. -->
      </balloon_text>       
...
<?php if ( $type_id != "mess" && $type_id != "ct" ) { ?>
    <graph gid="2">
      ...
      <alpha></alpha>
      <balloon_text>
        <![CDATA[<?=$graphUnit;?> <?php if ( $type_id != "demogdate" ) { ?><?=$word['datedef'];?> <?php } ?>{series}: {value} <?=$graphMeasure;?> (<?=$word['smoothed'];?>)]]>  <!-- [column(line).balloon.text] ({title} {value} {series} {description} {percents}) You can format any balloon text: {title} will be replaced with real title, {value} - with value and so on. You can add your own text or html code too. -->
      </balloon_text>                                         
    </graph>
<?php } ?>
  </graphs>
