...
header("Content-Disposition: filename=userlist_$title$addfiltname.csv");

print "$explain\n$word[t_email],$word[t_errcode]\n";
...
$res=mysql_query("select distinct email,error_code from bounced_back where message_id='$message_id'
                  ...
                  and group_id='$group_id' and project='maxima' order by email");

if ($res && mysql_num_rows($res)) { 
    while($row=mysql_fetch_array($res)) {
        print "$row[email],$row[error_code]\n";
    }
}
