...
$tplist=array("html_report","t_date","group","group_lower","total","t_messages","t_copies","mess_sent");
...
$tpl->assign("FORM_ACTION","mess_stat.php");
...
if ($report==1) {
    ...
    $tpl->define(array( "statistics" => "report_mess_stat.tpl"));
    $bgstyle1="bgwhite";
    $bgstyle2="bggray";
}
else {
    include "menugen.php";
    $tpl->define(array( "statistics" => "mess_stat.tpl"));
    $bgstyle1="bgwhite";
    $bgstyle2="bggray";
}
...
if ($_REQUEST["dostat"] || $report) {
    ...
    $query = mysql_query("select count(*),$whatc as d from messages 
                          where group_id='$group_id' and 
                          to_days(create_date)<='$end_daynum' and to_days(create_date)>='$start_daynum'
    ...
    logger($q,$group_id,"","uzenetek_statisztika","track");
    $query = mysql_query("select sum(mails),$what as d from track 
                          where group_id='$group_id' and 
                          to_days(date)<='$end_daynum' and to_days(date)>='$start_daynum' and date<='2006-12-16 12:09:57'
                          group by d order by d");
      if ($query && mysql_num_rows($query)) {
        while ($l=mysql_fetch_row($query)) {
          if ($view_mode=="days")
    ...
    $query = mysql_query("select sum(tlb_count),$whatc as d from messages 
                          where group_id='$group_id' and 
                          to_days(create_date)<='$end_daynum' and to_days(create_date)>='$start_daynum' and create_date>'2006-12-16 12:09:57'
                          group by d order by d");
      if ($query && mysql_num_rows($query)) {
        while ($l=mysql_fetch_row($query)) {
          if ($view_mode=="days")
