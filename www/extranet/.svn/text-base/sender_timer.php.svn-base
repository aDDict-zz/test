...
{
	$changes="";	
	$res=mysql_query("select name,base_id,sender_id,filter_id,stype,stime,notice,test_email,smod,sdays,active,sdate from sender_timer where id='$delete_id' and group_id='$group_id'");
    ...
    mysql_query("delete from sender_timer where id='$delete_id' and group_id='$group_id'");
}
...
$qmain="from sender_timer where test='no' and status!='ready' $gqpart 
        and not (stype in('single','now') and to_days(now())-to_days(sdate)>10)";
...
switch ($fsort) {
    ...
    case 3: $order = "order by stype,sdate,stime,id"; break;
    case 4: $order = "order by stype,sdate desc,stime desc"; break;
}
...
echo "<tr>
      <td class='bgkiemelt2' width=12%><span class=szovegvastag>$word[group]</span><span class='szoveg'> (<input type='checkbox' onclick=\"location='sender_timer.php?group_id=$group_id&allgroups=$antiallgroups';\"$agch>$word[st_allgroups])</span></td>
      ...
      <td class='bgkiemelt2' width=15%><span class=szovegvastag>&nbsp;</span></td>
      </tr>
      <tr>
      <td bgcolor=white align=right colspan='6'><span class=szoveg>
      <a href='$_MX_var->baseUrl/sender_timer_ch.php?group_id=$group_id'>$word[timer_new]</a>&nbsp;&nbsp;$viewlogs</span></td>
      </tr>\n";
if ($maxrecords) {
    ...
    if ($rst && mysql_num_rows($rst)) {
        ...
        while($row=mysql_fetch_array($rst)) {
            ...
            if ($index%2)
               ...
               $bgrnd="bgcolor=white";
            $actions="&nbsp;";
            $modform="<a href='$_MX_var->baseUrl/sender_timer_ch.php?group_id=$row[group_id]&timer_id=$row[id]'>$word[timer_change]</a>&nbsp;";
			$viewlog="<a href='#' onClick='window.open(\"$_MX_var->baseUrl/view_log.php?group_id=$group_id&timer_id=$row[id]\", \"fefem\", \"width=650,height=500,scrollbars=yes,resizable=yes\"); return false;'>Log</a>";
            $delname=ereg_replace("[\"'\r\n]","",$row["name"]);
            $delete="<a href='sender_timer.php?group_id=$row[group_id]&delete_id=$row[id]' onClick=\"return(confirm('$word[sender_timer_delete_confirm]: $delname'))\">$word[sender_timer_delete]</a>&nbsp;";
            ...
            $errdesc=$_MX_sender->GetStatus($row["id"],'active');
            if ($row["stype"]=="single" || $row["stype"]=="now") {
                $type.=$row["sdate"];
            }
            ...
            echo "<tr>
		          <td style='$errdesc[2]';><b>$group_name</b></td>
		          <td style='$errdesc[2]';>$title</td>
		          <td style='$errdesc[2]';>$base</td>
		          <td style='$errdesc[2]';>$type</td>
		          <td style='$errdesc[2]';>$active</td>
		          <td align=right style='$errdesc[2]';><span class=szoveg>$modform $delete $viewlog</span></td>
		          </tr>";
			if ($errdesc[0]!=null)
  				echo "<tr><td colspan='6' style='border-bottom:1px $_MX_var->main_table_border_color solid; $errdesc[1]'>". htmlspecialchars($errdesc[0]) ."</span></td></tr>\n";
        }
        printnavigation();
    }
}
...
<tr>
 ...
 if ($first>0)
 echo " 
            <td nowrap align='right'>              <a href='$_MX_var->baseUrl/sender_timer.php?$params&first=0'><img
                src='$_MX_var->baseUrl/$_MX_var->application_instance/gfx/down2.gif' width='20' height='14' border='0'></a>
              <a href='$_MX_var->baseUrl/sender_timer.php?$params&first=$OnePageLeft'><img
...
echo "
            <td nowrap align='right'> &nbsp;
              <a href='$_MX_var->baseUrl/sender_timer.php?$params&first=$OnePageRight'><img
                src='$_MX_var->baseUrl/$_MX_var->application_instance/gfx/up1.gif' width='20' height='14' border='0'></a>
              <a href='$_MX_var->baseUrl/sender_timer.php?$params&first=$LastPage'><img
