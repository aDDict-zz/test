...
$Amsortmmm=get_cookie("Amsortmmm");
$Amperpage=get_cookie("Amperpage");

$sortmmm = (isset($_GET['sortmmm']) || empty($Amsortmmm)) ? get_http('sortmmm',4) : $Amsortmmm;
...
//$active_userid = get_http('active_userid',0);
...
if ($deluser_id) {
    $old_membership="";
    $res=mysql_query("select membership from members where user_id='$deluser_id' and group_id='$group_id'");
    logger($query,$group_id,"","user_id=$deluser_id","members");                    
    if ($res && mysql_num_rows($res)) {
        if (mx_can_i_modify_membership(mysql_result($res,0,0))) {
        	$query="delete from members where user_id='$deluser_id' and group_id='$group_id'";
            mysql_query($query);
            logger($query,$group_id,"","user_id=$deluser_id","members");
			$query="delete from stattype_user where user_id='$deluser_id' and group_id='$group_id'";            
            mysql_query($query);
            logger($query,$group_id,"","user_id=$deluser_id","stattype");
			$query="delete from page_user where user_id='$deluser_id' and group_id='$group_id'";
            mysql_query($query);
            logger($query,$group_id,"","user_id=$deluser_id","page_user");
        }
    }
}

$qmain=" from user,members where user.id=members.user_id and members.group_id='$group_id'";
logger($qmain,$group_id,"","","users,members");
...
if ($maxrecords) {
    $rst=mysql_query("select user.name,user.email,members.* $qmain $order limit $first,$maxPerPage");
    $index = $first;
    if ($rst && mysql_num_rows($rst)) {
        ...
        echo "<td class='bgkiemelt2' align=left width=20%><span class=szovegvastag>$word[t_name]</span></td>
              ...
              <a href='#' onClick='window.open(\"moderators_popup.php?$parms\", \"moder\", \"width=500,height=250,scrollbars=yes,resizable=yes\"); return false;'>$word[newam]</a>&nbsp;</span></td>
              </tr>\n";
        while($row=mysql_fetch_array($rst)) {
            ...
            if (mx_can_i_modify_membership($row["membership"])) {
                $deluser="<a href='moderators.php?$parms&deluser_id=$row[user_id]' onClick=\"return(confirm('$word[sure_delete]'))\">$word[delete]</a>&nbsp;";
                $modusera="<a href='#' onClick='window.open(\"moderators_popup.php?$parms&user_id=$row[user_id]\", \"moder$row[id]\", \"width=500,height=250,scrollbars=yes,resizable=yes\"); return false;'>";
                $moduserb="</a>";
                $actions=$deluser;
            }
            ...
            echo "<tr>
		          ...
		          <td $bgrnd align=left width=25%><span class=szoveg>$modusera$row[email]$moduserb</span></td>
