...
<? if (!isset($_MX_iframe) && !isset($_MX_popup)): ?>
    ...
    $menu=0;
    if ($group_id && $weare) {
        $r1=mysql_query("select * from groups where id='$group_id'"); //echo "select * from groups where id='$group_id'"; die();
        if ($r1 && mysql_num_rows($r1)) {
            $groupdata=mysql_fetch_array($r1); //print_r($groupdata); die();
            //$type_membership=$groupdata["type_membership"];
    ...
    $request_uri=ereg_replace("^\/[a-zA-Z0-9_]+\/","/","http://localhost/maxima");
    ...
    <div id="header_right_stuff" class="fr" style="width:750px;margin-top:10px">
        ...
        $quq="select count(m.id) as linecount, sum(m.send_plan) as sum_send_plan, sum(m.tlb_count) as sum_tlb_count
            ...
            and m.group_id=g.id and g.id=p.group_id and p.user_id='$active_userid' and p.membership in ('owner','moderator')"; /*echo "select count(m.id) as linecount, sum(m.send_plan) as sum_send_plan, sum(m.tlb_count) as sum_tlb_count
            from messages m,groups g,members p where m.tlb_finished='no' and m.send_plan>0 
            and m.group_id=g.id and g.id=p.group_id and p.user_id='$active_userid' and p.membership in ('owner','moderator')"; die();*/
        $qur=mysql_query($quq);
        if ($quk=mysql_fetch_array($qur)) {
          if ($quk["linecount"]) {
            ...
            <div class="fr">
                <input type="checkbox" id="autoopen" <?if($_COOKIE["proclist_autoopen"] == 1){?>checked="checked"<?}?> />
    ...
    <div class="menu<?if($_COOKIE["proclist_autoopen"] == 1){?> menubg<?}?>">
    <?       
